...
likertAgeData <- function(data, years, question, religions){
    ...
    x3 <- data %>% dplyr::select(age, answer) %>% filter(!is.na(age)) %>% mutate(age_bin = binage(age)) %>% dplyr::select(-age) %>% group_by(age_bin, answer) %>% dplyr::summarise(n()) %>% spread(key = answer, value = `n()`)
    x3 <- x3 %>% dplyr::select(-DK) #removing DK variable
    x3
    x3 <- x3[age_answer_order] #rearrange so responses go in right order
